...
never appear in a fixture.
    ...
    python -m unittest discover -s .codex/skills/latex-cv-tailor/tests
"""
...
def workspace_tempdir():
    """Create a sandbox-writable temporary directory with inherited ACLs."""
    root = WORKSPACE_ROOT / f".codex-test-{uuid4().hex}"
    root.mkdir()
    try:
        yield str(root)
    finally:
...
class SkillIsolation(unittest.TestCase):
    ...
    def test_runtime_imports_resolve_to_shared_scripts(self):
        for module in (
            ...
            self.assertEqual(Path(module.__file__).resolve().parent, SCRIPTS.resolve())

    def test_codex_copy_has_no_claude_path_dependency(self):
        checked = [SKILL_ROOT / "SKILL.md"]
